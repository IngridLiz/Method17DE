# -*- coding: utf-8 -*-
"""
Created on Mon Feb 17 08:58:50 2025
...
# Conversions2 (nmol/(ml*min) = uM/min)
uMmM = 1000.            # (uM/mM)
...
Jhyd_v=[]
def fluxes(v): 
    [adpc, adpm, akg, atpc, atpm, cac, cam, cit, fum, isoc, mal, nadm, nadhm, oaa, psi, scoa, suc ]= v
    ...
    Jidh = Vmaxidh / (1. + (hm / kh1) + (kh2 / hm) + (((kmisoc / isoc) ** ni) / ((1. + adpm / kaadp) * (1. + cam / kaca))) + (
            kmnad / nadm) * (1. + (nadhm / kinadh)) + ((((kmisoc / isoc) ** ni )* (kmnad / nadm) * (1. + (nadhm / kinadh))) / ((1. + (adpm / kaadp)) * (1. + (cam / kaca)))))                                                 # (Cortassa et al., 2003)  
    ...
    Jmdh = Vmdh * (mal * nadm - (oaa * nadhm / Keqmdh)) / (
                (1. + mal / kmmal) * (1. + nadm / kmnadmdh) + ((1. + oaa / kmoaamdh) * (
                    1. + nadhm / kmnadhmdh)) - 1.)                                                                                                      # (Berndt et al., 2015)
    ...
    Ares = kres * np.sqrt(nadhm / nadm )    #cte
    ...
    flux = [Jcs_v.append(Jcs), Jaco_v.append(Jaco),  Jidh_v.append(Jidh), Jkgdh_v.append(Jkgdh) , 
    ...
    return   Jaco, Jant, Jcs, Jerout, Jf1, Jfh, Jhl, Jidh, Jkgdh, Jmdh, Jncx, Jo, Jsdh, Jserca, Jsl, Juni, Jhyd, flux;
...
def metcal_ode(s, v):
    ...
    # Compute fluxes
    Jaco, Jant, Jcs, Jerout, Jf1, Jfh, Jhl, Jidh, Jkgdh, Jmdh, Jncx, Jo, Jsdh, Jserca, Jsl, Juni, Jhyd, flux = fluxes(v)
    ...
    dvds[11] = -(-Jo + Jidh + Jkgdh + Jmdh )  # nadm mM/s
    dvds[12] = -Jo + Jidh + Jkgdh + Jmdh   # nadhm mM/s
...
y1= [adpc0, adpm0, akg0, atpc0, atpm0, cac0, cam0, cit0, fum0, isoc0, mal0, nadm0, nadhm0, oaa0, psi0, scoa0, suc0 ]
     
# Solve the ODE
res = solve_ivp(metcal_ode, [0, sec], y1, method='LSODA', atol=1.e-15, rtol=1.e-13, t_eval=ts)
#res = solve_ivp(metcal_ode, [0, sec], y1, method='LSODA', atol=1.e-5, rtol=1.e-5, t_eval=ts)
#res = solve_ivp(metcal_ode, [0, sec], y1, method='DOP853', atol=1.e-2, rtol=1.e-2, t_eval=ts)
